...
### Ship Now
- [ ] talli: v3.5.4 build 202607141116 submitted for App Store review (submission 9952a47b, pending). Widget/app version mismatch fixed (ITMS-90473, commit 46f3eca) + DerivedData build-number mismatch fixed (clean wipe + rebuild). Follow-up bug found + fixed server-side: Messages/Home tab showing scraped portal nav-spam ("Notifications", "Payment Info") as fake messages — fixed via shared prefix-regex filter on /api/mobile + web (commit 7b23585), live via Vercel now, but NOT yet in the submitted iOS build (needs a future version bump). Open: user-reported iOS login-screen text-contrast bug (dark-on-dark title/subtitle) — SwiftUI source (ContentView.swift LoginScreen, .preferredColorScheme(.dark)) looks correct on inspection, root cause not found; possibly stale installed build or device accessibility setting, needs investigation next session.
