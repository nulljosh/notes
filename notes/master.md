...
All notes in one place. Updated 2026-07-14, Tuesday late evening.

**Latest (2026-07-14 nightly wrap — Talli 3.5.5 in flight):** Talli iOS version bumped to 3.5.5 and uploaded to App Store Connect (build 3.5.5 processing). Archive, export, and upload completed successfully. The ship-ios workflow's final publish/submit-for-review step errored on a path issue (cosmetic, not the build itself) — review submission needs resuming or manual confirmation. Meanwhile, Quotable nav logo made theme-adaptive via inline SVG using currentColor tied to --text, shipping light in dark mode and dark in light mode across web/iOS/macOS. Attempted push to nulljosh/quotable blocked by 403: repo is archived (read-only) on GitHub, needs unarchive before commits can land. NYC web title screen fixed: stopped auto-skipping into new game by calling showMenu() instead of startGame() in main.js init() (commit 7279d04). Books, dotfiles, and roadmap updates (echo/healstack/spark) all committed + pushed cleanly. Dotfiles asc-aso-audit skill expanded with optional Phase 3 pulling competitor signals from AppSigma MCP. Books synced tokens.css to portfolio's fredrika theme. 12 repos touched tonight (bcgd, books, dotfiles, echo, healstack, lexly, notes, nyc, quotable, spark, talli, notes) with 35+ commits total plus journal + wiki deploys. Quotable repo unarchive is a hard blocker for shipping the theme-adaptive nav.
