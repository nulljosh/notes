...
All notes in one place. Updated 2026-07-14, Tuesday night.

**Latest (2026-07-14 nightly wrap — AI Investing book, Talli 3.5.5 in flight):** AI Investing for Dummies book summarization progressing: chapters 4-6 (Large-Cap AI Stocks, Small-Cap AI Stocks and IPOs, Diversifying with ETFs and Mutual Funds) and 7-8 (Robo-Advisors, Fundamental and Technical Analysis) completed as summaries tonight. Raw HEICs retained pending final merge into master summary and repo sync (7 chapters remaining: 9, 10-11, 12, 13-15, 16, Remaining). Talli iOS v3.5.5 uploaded to ASC (build processing). Ship-ios workflow publish step errored on path issue (cosmetic, not build) — review needs manual confirmation. Quotable nav logo theme-adaptive via inline SVG using currentColor (web/iOS/macOS). Attempted push to nulljosh/quotable blocked by 403: archived/read-only, needs unarchive. NYC web title screen fixed: stopped auto-skipping into new game (commit 7279d04). Books, dotfiles, roadmaps all committed + pushed. Dotfiles asc-aso-audit skill expanded with Phase 3 AppSigma competitor signals. Books synced tokens.css to fredrika theme. 12 repos touched (bcgd, books, dotfiles, echo, healstack, lexly, notes, nyc, quotable, spark, talli) plus nightly journal/wiki updates. Quotable unarchive is blocker for theme-adaptive nav.
...
- Books / AI Investing: Manual book summarization in progress. Completed chapter summaries for 4-6 (Large-Cap AI, Small-Cap AI/IPOs, Diversifying with ETFs) and 7-8 (Robo-Advisors, Fundamental/Technical Analysis). Raw HEICs retained per policy until all chapters done, then merge into master summary, sync via ~/Documents/Code/books/sync-summaries.sh, commit+push, then delete raws. Remaining: chapters 9, 10-11, 12, 13-15, 16, Remaining folder. Progress: 2 of ~8 remaining chapters complete.
...
- [ ] ai-investing-for-dummies: Book summarization in progress (2026-07-14). Chapters 4-6 and 7-8 summaries completed. Remaining: chapters 9, 10-11, 12, 13-15, 16, Remaining folder. After all chapters done: merge into ai-investing-for-dummies-summary.md, run ~/Documents/Code/books/sync-summaries.sh, commit+push, delete raw HEICs
