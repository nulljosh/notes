...
All notes in one place. Updated 2026-07-15, after Tuesday nightly wrap.

**Latest (2026-07-14 late nightly wrap — cleanup pass across 8 repos, 55 commits):** Cleanup sweep across eight repositories. Talli v3.5.5 bumped with fresh archive/export/upload to ASC (processing). Quotable nav logo made theme-adaptive via currentColor (light in dark mode, dark in light mode) shipping across web/iOS/macOS (push blocked on repository unarchive). NYC title screen fixed to show menu before auto-skipping into game (commit 7279d04). Lexly hardened lesson-completion storage against id collisions and wired launch screen to AppIcon display with architecture.svg added. Grapher whitepaper added documenting visualization pipeline. Echo xcodeproj updated and speak-back + voices + paywall-gating feature stashed to roadmap. Books iOS app screenshot added to README showing library list with AI Investing summary badge live. Dotfiles asc-aso-audit skill extended with Phase 3 for AppSigma competitor signals. All changes across spark, nyc, journal, labs, quotable, talli, echo, dotfiles, books, notes, grapher committed and pushed. 11 repos touched, 55 commits total. Journal updated and deployed to Vercel.
