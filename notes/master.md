...
| spark | v2.1.0 live, Mac build 1.0.0 uploaded to ASC (processing), auth consolidated (10/12 fns), dark-mode text fix. ASC app icon blocker root-caused (2026-07-07): distribution provisioning profile missing for com.heyitsmejosh.spark/.spark.mac (distribution certs exist, profiles don't); next: asc-signing-setup to generate profiles, then re-upload builds | iOS screenshots + metadata + submit; generate distribution profiles via asc-signing-setup |
...
| portfolio (nulljosh.github.io) | Web live at heyitsmejosh.com (sage.me theme dark mode + perf live), iOS v1.0.0 app (Swift mirror of site, same tokens, 9/10 ASC blockers cleared 2026-07-06 nightly, availability pending then ready to submit to App Review, ASC id 6788180394). iOS app icon fixed (2026-07-07): regenerated full-bleed edge-to-edge from icon.svg (removed baked-in corner rounding, iOS applies its own mask), uncommitted | Commit icon fix, click availability dashboard toggle, then submit to App Review |
...
| healstack (dose) | v2.2.0 live, widget last-dose sync fixed, iOS marketing icon alpha stripped. ASC app icon blocker root-caused (2026-07-07): distribution provisioning profile missing for com.heyitsmejosh.dose (distribution cert exists, profile doesn't); next: asc-signing-setup to generate profile, then re-upload v2.3.3 build 2 | Mobile nav, health grading; generate distribution profile via asc-signing-setup |
...
| books | Web live at books.heyitsmejosh.com (ML for Dummies chapters 13–20 summary complete), iOS + macOS WKWebView wrapper apps (builds + auto-signing verified), ASC app records + bundle IDs registered. ASC app icon blocker root-caused (2026-07-07): distribution provisioning profiles missing for com.heyitsmejosh.books/.books.mac (distribution certs exist, profiles don't); next: asc-signing-setup to generate profiles, then archive + upload builds | Generate distribution profiles via asc-signing-setup, then archive + upload to TestFlight |
...
- Lexly: landed marketing landing page at `/index.html` with course highlights and public-facing intro, moved app shell to `/app/`, unified header styling between landing and app, confirmed native iOS/macOS SwiftUI accent (#5B9BD5) matches web parity, closing roadmap item
