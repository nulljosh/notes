...
All notes in one place. Updated 2026-07-09 (Epiphany Sign in with X Twitter OAuth + landscape revert, portfolio Claude Design System scaffolded).
...
- Epiphany: two commits pushed—reverted landscape orientation flag (needs full per-screen layout pass before that's safe), and shipped Sign in with X (Twitter) OAuth to web dashboard (complements GitHub + Apple OAuth)
