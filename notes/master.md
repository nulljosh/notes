...
All notes in one place. Updated 2026-07-09 (Epiphany v2.6.1 TUI dashboard shipped, Talli v3.5.4 TestFlight uploaded + icon rebrand blue/alpha-fix, Spark v2.1.0 YC RFS Inspiration feature added, books library tracking updated).
...
| spark | v2.1.0 live (Mac build 1.0.0 processing), auth consolidated (10/12 fns), dark-mode text fix, YC RFS Inspiration strip added (scrapes ycombinator.com/rfs with 12h in-memory cache, carousel display linking to full RFP list, 2026-07-09, zero additional function cost). App Group linking attempted via developer.apple.com's App ID capability config (2026-07-07) but entitlement still comes back empty in freshly regenerated provisioning profiles—unresolved, genuinely blocked (needs re-investigation: verify modal still shows group checked, wait longer, or re-toggle capability). | iOS screenshots + metadata remain; App Group entitlement issue must resolve before archive/export/upload |
...
- Spark: added YC RFS Inspiration strip that scrapes ycombinator.com/rfs into api/ai.js with a new type=rfs route using 12-hour in-memory cache. Displays Y Combinator's Requests for Startups in a carousel strip linking out to the full RFP list. Zero additional function cost—stays under Vercel Hobby 12-function cap. Committed and pushed.
